...
export const validationRegisterationData = (data: any, userType: "user" | "seller") => {
...
export const checkOtpRestrictions = async (email: string, next: NextFunction) => {
    if (await redis.get(`otp_lock:${email}`)) {
        throw new ValidationError("Acoount Locked due to Multiple Failed Attempts! Try Again After 30 Minutes.");
    };
    if (await redis.get(`otp_spam_lock:${email}`)) {
        throw new ValidationError("Too many OTP requests! Please wait 1 Hour before requesting again.");
    };
    if (await redis.get(`otp_cooldown:${email}`)) {
        throw new ValidationError("Please wait 1 Minute before requesing a new OTP!");
    };
};
...
export const handleForgotPassword = async (req: Request, res: Response, next: NextFunction, userType: "user" | "seller") => {
    try {
        ...
        // Generate OTP
        await sendOtp(email, user.name, "forgot-passwrd-user-mail");
